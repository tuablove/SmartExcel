 ...
 */
public class ExcelDynamicTemplateWriter<T> extends AbstractExcel implements IExcelCellStyleCreator {
    private static final Logger logger = LoggerFactory.getLogger(ExcelDynamicTemplateWriter.class);
     ...
     * @throws Exception
     */
    public void write(List<T> dataList, ExcelWriteDynamicTemplateConfig config) throws Exception {
        ...
        try (FileInputStream fileIs = new FileInputStream(config.getTemplateFilePath())) {
            ...
            Map<String, WriteDataFieldDefinition> titleDefinitionMap = new HashMap<>();
            for (WriteDataFieldDefinition dataField : dataFields) {
                for (String titleName : dataField.getTitleNames()) {
            ...
            Row titleRow = sheet.getRow(config.getTitleRowIndex());
            Map<Integer, String> columnTitleMap = new HashMap<>();
            for (int i = titleRow.getFirstCellNum(); i < titleRow.getLastCellNum(); i++) {
                Object cellValue = CellUtils.getCellValue(titleRow.getCell(i));
                if (cellValue!=null &&  StringUtils.isNotBlank(cellValue.toString())) {
                    columnTitleMap.put(i, cellValue.toString().trim());
                }
            }
            ...
            for (int i = 0; i < dataList.size(); i++, rowIndex++) {
                ...
                for (Map.Entry<Integer, String> columnTitle : columnTitleMap.entrySet()) {
                    Integer c = columnTitle.getKey();
                    ...
                    WriteDataFieldDefinition dataField = titleDefinitionMap.get(columnTitle.getValue());
                    if (dataField != null) {
     ...
     * @param clazz
     */
    public ExcelDynamicTemplateWriter(Class<T> clazz) {
        this.clazz = clazz;
    }
